...
namespace ASCOM.Utilities
{
    ...
    public class TraceLogger : ITraceLogger, ITraceLoggerExtra, IDisposable
    {
        ...
        private string userHome { get => Environment.GetFolderPath(Environment.SpecialFolder.Personal); }
        ...
        public string LogFileName
        {
            get
            {
                ...
                string logfile = g_LogFileName.Trim();
                if (LogFileIdx > 0)
                    logfile = $"{logfile}-{LogFileIdx}";
                return !Path.HasExtension(logfile) ? $"{logfile}.{_defaultExt}" :logfile;
            }
        }
        public string LogDir
        {
            get
            {
                return !String.IsNullOrWhiteSpace(g_LogDir)
                    ? g_LogDir : $"{userHome}\\ASCOM\\Logs\\{DateTime.Now.ToString("yyyy-MM-dd")}";
            }
            private set
            {
                string newPath = value?.Trim() ?? "";
                if (newPath == "")
                    newPath = $"{userHome}\\ASCOM\\Logs\\{DateTime.Now.ToString("yyyy-MM-dd")}";
        ...
        public TraceLogger(string logFileName, string logFileType)
        {
            ...
            LogDir = $"{userHome}\\ASCOM\\Logs\\{DateTime.Now.ToString("yyyy-MM-dd")}";
            CreateLogFile();
        }
        ...
        public void LogIssue(string Identifier, string Message)
        {
            try
            {
                ObtainLock("LogIssue", $"\"{Identifier}\", \"{Message}\"");
                if (!this.Enabled)
                    return;
                if (this.g_LogFile == null)
                    this.CreateLogFile();
        ...
        public void LogMessageHex(string Identifier, string Message)
        {
            bool HexDump = true;
            string p_Msg = Message;
            try
            {
                ObtainLock("LogMessage", "\"" + Identifier + "\", \"" + Message + "\", " + HexDump.ToString() + "\"");
                if (this.g_LineStarted)
                    this.LogFinish(" ");
                if (!this.Enabled)
                    return;
        ...
        public void LogStart(string Identifier, string Message)
        {
            try
            {
                this.ObtainLock("LogStart", "\"" + Identifier + "\", \"" + Message + "\"");
                if (this.g_LineStarted)
                {
            ...
            public void LogContinue(string Message)
        {
            try
            {
                this.ObtainLock("LogContinue", "\"" + Message + "\"");
                if (!this.g_LineStarted)
                {
                ...
                else
                {
                    ...
                    if (this.g_LogFile == null)
                        this.CreateLogFile();
                    string pMsg = this.MakePrintable(Message, false);
                    write(pMsg);
                }
            }
            finally
        ...
        public void LogFinish(string Message)
        {
            try
            {
                this.ObtainLock("LogFinish", "\"" + Message + "\"");
                if (!this.g_LineStarted)
                {
        ...
        private void write(string msg,bool newLine=false)
        {
            if (!Enabled || g_LogFile==null)
                return;
            ObtainLock("write", $"newLine={newLine}, msg=[suppressed]");
            try
            {
                if (newLine)
                    g_LogFile.WriteLine(msg);
                else
                    g_LogFile.Write(msg);
                if(!g_LogFile.AutoFlush)
                    g_LogFile.Flush();
                if (EchoConsole)
                {
                    if (newLine)
        ...
        #region TraceLoggerExt

        public void LogMessage<T>(string Identifier, T Message)
            where T : struct
        {
            LogMessage(Identifier, Message.ToString());
        }

        public void LogMessage<T>(string Identifier, string Message, bool hexDump)
            where T : struct
        {
            String msgString = hexDump
                ? MakeHex(Message)
                : Message;
            LogMessage(Identifier, msgString);
        }

        public void LogMessage(string Identifier, String fmt, params object[] args)
