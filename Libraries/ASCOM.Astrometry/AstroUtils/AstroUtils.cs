  ...
  public class AstroUtils : IAstroUtils, IDisposable
  {
    ...
    public double JulianDateUtc
    {
      get
      {
        ...
        double num = this.Nov31.JulianDate(Convert.ToInt16(utcNow.Year), Convert.ToInt16(utcNow.Month), Convert.ToInt16(utcNow.Day), utcNow.TimeOfDay.TotalHours);
        this.TL.LogMessage("JulianDateUtc", "Returning: " + Conversions.ToString(num) + " at UTC: " + Strings.Format((object) utcNow, "dddd dd MMMM yyyy HH:mm:ss.fff"));
        return num;
      }
    }
    ...
    public int LeapSeconds
    {
      get
      {
        return this.UtcTaiOffset;
      }
      set
      {
        this.UtcTaiOffset = value;
        this.RegAccess.WriteProfile("Astrometry", "Leap Seconds", value.ToString());
      }
    }

    public AstroUtils()
    {
      this.TL = new TraceLogger("", "AstroUtils");
      this.TL.Enabled = RegistryCommonCode.GetBool("Trace Astro Utils", false);
      this.Utl = new Util();
      this.Nov31 = new NOVAS31();
      this.RegAccess = new RegistryAccess();
      this.TL.LogMessage("New", "AstroUtils created Utilities component OK");
      this.UtcTaiOffset = Conversions.ToInteger(this.RegAccess.GetProfile("Astrometry", "Leap Seconds", 36.ToString()));
      this.TL.LogMessage("New", "Leap seconds: " + Conversions.ToString(this.UtcTaiOffset));
      this.TL.LogMessage("New", "Finished initialisation OK");
    }
    ...
    protected virtual void Dispose(bool disposing)
    {
      if (!this.disposedValue)
      {
        ...
        {
          this.Nov31.Dispose();
          this.Nov31 = (NOVAS31) null;
        }
        if (this.RegAccess != null)
        {
          this.RegAccess.Dispose();
          this.RegAccess = (RegistryAccess) null;
        }
      }
    ...
    {
      double Hours = this.Range(HA, -12.0, true, 12.0, true);
      this.TL.LogMessage("ConditionHA", "Conditioned HA: " + this.Utl.HoursToHMS(HA, ":", ":", "", 3) + " to: " + this.Utl.HoursToHMS(Hours, ":", ":", "", 3));
      return Hours;
    }

    public double ConditionRA(double RA)
    {
      double Hours = this.Range(RA, 0.0, true, 24.0, false);
      this.TL.LogMessage("ConditionRA", "Conditioned RA: " + this.Utl.HoursToHMS(RA, ":", ":", "", 3) + " to: " + this.Utl.HoursToHMS(Hours, ":", ":", "", 3));
      return Hours;
    }

    public double DeltaT()
    {
      double julianDate = this.Utl.JulianDate;
      double num = DeltatCode.DeltaTCalc(julianDate);
      this.TL.LogMessage("DeltaT", "Returned: " + Conversions.ToString(num) + " at Julian date: " + Conversions.ToString(julianDate));
      return num;
    }
    ...
    public double JulianDateTT(double DeltaUT1)
    {
      ...
      if (DeltaUT1 != 0.0)
      {
        TimeSpan timeSpan1 = TimeSpan.FromSeconds(this.DeltaT());
        TimeSpan timeSpan2 = TimeSpan.FromSeconds(DeltaUT1);
        dateTime = utcNow.Add(timeSpan2).Add(timeSpan1);
      }
      else
        dateTime = utcNow.Add(TimeSpan.FromSeconds((double) this.UtcTaiOffset + 4023.0 / 125.0));
      double num = this.Nov31.JulianDate(Convert.ToInt16(dateTime.Year), Convert.ToInt16(dateTime.Month), Convert.ToInt16(dateTime.Day), dateTime.TimeOfDay.TotalHours);
      this.TL.LogMessage("JulianDateTT", "Returning: " + Conversions.ToString(num) + "at TT: " + Strings.Format((object) dateTime, "dddd dd MMMM yyyy HH:mm:ss.fff") + ", at UTC: " + Strings.Format((object) utcNow, "dddd dd MMMM yyyy HH:mm:ss.fff"));
      return num;
    }
    ...
    public double JulianDateUT1(double DeltaUT1)
    {
      ...
      {
        double num = DeltatCode.DeltaTCalc(this.Nov31.JulianDate(Convert.ToInt16(utcNow.Year), Convert.ToInt16(utcNow.Month), Convert.ToInt16(utcNow.Day), utcNow.TimeOfDay.TotalHours));
        dateTime = utcNow.Add(TimeSpan.FromSeconds((double) this.UtcTaiOffset + 4023.0 / 125.0)).Subtract(TimeSpan.FromSeconds(num));
      }
      double num1 = this.Nov31.JulianDate(Convert.ToInt16(dateTime.Year), Convert.ToInt16(dateTime.Month), Convert.ToInt16(dateTime.Day), dateTime.TimeOfDay.TotalHours);
      this.TL.LogMessage("JulianDateUT1", "Returning: " + Conversions.ToString(num1) + "at UT1: " + Strings.Format((object) dateTime, "dddd dd MMMM yyyy HH:mm:ss.fff") + ", at UTC: " + Strings.Format((object) utcNow, "dddd dd MMMM yyyy HH:mm:ss.fff"));
      return num1;
    }
    ...
    public double UnRefract(OnSurface Location, RefractionOption RefOption, double ZdObs)
    {
      ...
      {
        checked { ++num1; }
        num2 = ZdObs1 - this.Nov31.Refract(Location, RefOption, ZdObs1);
        ZdObs1 += ZdObs - num2;
        this.TL.LogMessage("Unrefract", Conversions.ToString(num1) + ": " + Conversions.ToString(num2) + " " + Conversions.ToString(ZdObs1));
      }
      while (!(num1 == 20 | num2 == ZdObs));
      this.TL.LogMessage("Unrefract", "Final: " + Conversions.ToString(num1) + ", Unrefracted zenith distance: " + Conversions.ToString(ZdObs1));
      return ZdObs1;
    }

    public double CalendarToMJD(int Day, int Month, int Year)
    {
      return this.Nov31.JulianDate(Convert.ToInt16(Year), Convert.ToInt16(Month), Convert.ToInt16(Day), 0.0) - 2400000.5;
    }

    public double MJDToOADate(double MJD)
    {
      return this.Utl.DateJulianToLocal(MJD + 2400000.5).ToOADate();
    }

    public DateTime MJDToDate(double MJD)
    {
      return this.Utl.DateJulianToLocal(MJD + 2400000.5);
    }

    public string FormatMJD(double MJD, string PresentationFormat)
    {
      return Strings.Format((object) this.MJDToDate(MJD), PresentationFormat);
    }

    public double DeltaUT(double JulianDate)
    {
      double num = (double) this.UtcTaiOffset + 4023.0 / 125.0 - DeltatCode.DeltaTCalc(JulianDate);
      this.TL.LogMessage("DeltaUT", "Returning: " + Conversions.ToString(num) + " at Julian date: " + Conversions.ToString(JulianDate));
      return num;
    }

    public string FormatJD(double JD, string PresentationFormat)
    {
      this.TL.LogMessage("FormatJD", "JD, PresentationFormat: " + Conversions.ToString(JD) + " " + PresentationFormat);
      double num = JD - 2451545.0;
      DateTime dateTime1 = new DateTime(2000, 1, 1, 12, 0, 0);
      DateTime dateTime2 = dateTime1.AddDays(num);
      this.TL.LogMessage("FormatJD", "  DaysSinceJ2000, J2000Date, ActualDate: " + Conversions.ToString(num) + " " + dateTime1.ToString() + " " + dateTime2.ToString());
      string str = Strings.Format((object) dateTime2, PresentationFormat);
      this.TL.LogMessage("FormatJD", "  Result: " + str);
      return str;
    }
    ...
    public ArrayList EventTimes(EventType TypeofEvent, int Day, int Month, int Year, double SiteLatitude, double SiteLongitude, double SiteTimeZone)
    {
      ...
      catch (Exception ex)
      {
        ...
        this.TL.LogMessageCrLf("EventTimes", ex.ToString());
        throw;
      }
      double JD = this.Nov31.JulianDate(checked ((short) Year), checked ((short) Month), checked ((short) Day), 0.0) - SiteTimeZone / 24.0;
      Location.Latitude = SiteLatitude;
      Location.Longitude = SiteLongitude;
      double num1 = this.Nov31.Refract(Location, RefractionOption.StandardRefraction, 90.0);
      double Hour = 1.0;
      bool flag3=false;
      do
      {
        ASCOM.Astrometry.AstroUtils.AstroUtils.BodyInfo bodyInfo1 = this.BodyAltitude(TypeofEvent, JD, Hour - 1.0, SiteLatitude, SiteLongitude);
        ASCOM.Astrometry.AstroUtils.AstroUtils.BodyInfo bodyInfo2 = this.BodyAltitude(TypeofEvent, JD, Hour, SiteLatitude, SiteLongitude);
        ASCOM.Astrometry.AstroUtils.AstroUtils.BodyInfo bodyInfo3 = this.BodyAltitude(TypeofEvent, JD, Hour + 1.0, SiteLatitude, SiteLongitude);
        double num2;
        double num3;
        double num4;
        switch (TypeofEvent)
    ...
    private ASCOM.Astrometry.AstroUtils.AstroUtils.BodyInfo BodyAltitude(EventType TypeOfEvent, double JD, double Hour, double Latitude, double Longitude)
    {
      ...
      int num1 = (int) this.Nov31.Place(JdHigh + DeltaT * 1.15740740740741E-05, CelObject, Location, DeltaT, CoordSys.EquinoxOfDate, Accuracy.Full, ref Output);
      bodyInfo.Distance = Output.Dis * 149597870.691;
      double Gst=0;
      this.Nov31.SiderealTime(JdHigh, 0.0, DeltaT, GstType.GreenwichApparentSiderealTime, ASCOM.Astrometry.Method.EquinoxBased, Accuracy.Full, ref Gst);
      double num2 = 15.0 * (this.Range(Gst + Longitude * (1.0 / 15.0), 0.0, true, 24.0, false) - Output.RA);
    ...
    public double MoonIllumination(double JD)
    {
      ...
      Location.OnSurf = onSurface;
      Location.Where = ObserverLocation.EarthGeoCenter;
      int num1 = (int) this.Nov31.Place(JD + DeltaT * 1.15740740740741E-05, CelObject, Location, DeltaT, CoordSys.EquinoxOfDate, Accuracy.Full, ref Output2);
      CelObject.Name = "Sun";
      CelObject.Number = Body.Sun;
      CelObject.Star = catEntry3;
      CelObject.Type = ASCOM.Astrometry.ObjectType.MajorPlanetSunOrMoon;
      int num2 = (int) this.Nov31.Place(JD + DeltaT * 1.15740740740741E-05, CelObject, Location, DeltaT, CoordSys.EquinoxOfDate, Accuracy.Full, ref Output1);
    ...
    public double MoonPhase(double JD)
    {
      ...
      CelObject.Type = ASCOM.Astrometry.ObjectType.MajorPlanetSunOrMoon;
      int num2 = (int) this.Nov31.Place(JD + DeltaT * 1.15740740740741E-05, CelObject, Location, DeltaT, CoordSys.EquinoxOfDate, Accuracy.Full, ref Output1);
      return this.Range((Output2.RA - Output1.RA) * 15.0, -180.0, false, 180.0, true);
    }
